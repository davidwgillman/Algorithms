...
public class DoubleHashST<Key, Value> {
    ...
    public void put(Key key, Value val) {
        ...
        int indexOfMax = -1; // hold index of larger symbol table
        if (n[0]>n[1]) {
            indexOfMax = 0;
        }
        else {
        ...
        if (n[indexOfMax] >= m/2) resize(2*m);
    ...
    public void delete(Key key) {
        ...
        if (n[indexOfMax] > 0 && n[indexOfMax] <= m/8) resize(m/2);
