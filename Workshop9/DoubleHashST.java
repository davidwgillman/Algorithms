...
public class DoubleHashST<Key, Value> {
    ...
    public void put(Key key, Value val) {
    	LinkedList<Key> why = new LinkedList<Key>();
        ...
        if ((double)n[0]/m > 0.5 || (double)n[1]/m > 0.5) {
        	resize(2*m);
        ...
        */
        ...
        if(n[0] < n[1]) {
        	...
        	j = 0;
        		while(true) {
        			this.setCompCounter(this.getCompCounter() + 1);
        			if(keys[j][i] == null) {
                		keys[j][i] = key;
                		vals[j][i] = val;
                		n[j]++;
        			...
        			Key OldKey = keys[j][i];
            		Value OldVal = vals[j][i];
        			if(why.contains(OldKey)) {
        				rehash();
        				this.setRehashed(true);
        				//put(OldKey, OldVal);
        			}
        			why.add(OldKey);
        			keys[j][i] = key;
    ...
    public void delete(Key key) {
        ...
        if(n[1] < n[0]) {
        	 if (this.isEmpty() != true && n[0]/m < (double)(1/8)) {
             	resize(m/2);
             	this.setResized(true);
             }
        }
        if(n[0] < n[1]) {
        	if (this.isEmpty() != true && n[1]/m < (double)(1/8)) {
            	resize(m/2);
            	this.setResized(true);
            }
       }
	...
	public void setCompCounter(int compCounter) {
